...
#define FLIP_TRANS(type) (((type) == TransType::kRise) ? TransType::kFall : TransType::kRise)
...
inline double ConvertCapUnit(CapacitiveUnit src_unit, CapacitiveUnit snk_unit, double cap)
{
  double converted_cap = cap;
  if (src_unit != snk_unit) {
    if ((src_unit == CapacitiveUnit::kFF) && (snk_unit == CapacitiveUnit::kPF)) {
      converted_cap = FF_TO_PF(cap);
    } else if ((src_unit == CapacitiveUnit::kPF) && (snk_unit == CapacitiveUnit::kFF)) {
...
class LibertyArc : public LibertyObject
{
 public:
  ...
  void set_timing_type(std::string& timing_type);
 ...
 *
 */
#define FOREACH_CELL_PORT(cell, port) for (ieda::LibertyCellPortIterator iter(cell); iter.hasNext() ? port = (iter.next()), true : false;)
...
#define FOREACH_CELL_TIMING_ARC_SET(cell, timing_arc_set) \
  for (ieda::LibertyCellTimingArcSetIterator iter(cell); iter.hasNext() ? timing_arc_set = (iter.next()), true : false;)
...
#define FOREACH_POWER_ARC_SET(cell, power_arc_set) \
  for (ieda::LibertyCellPowerArcSetIterator iter(cell); iter.hasNext() ? power_arc_set = (iter.next()), true : false;)
 ...
 *
 */
#define FOREACH_LIB_CELL(lib, cell) for (ieda::LibertyCellIterator iter(lib); iter.hasNext() ? cell = (iter.next()), true : false;)
